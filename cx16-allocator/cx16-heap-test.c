...
void main() {
	...
	char* p1_data = (char*)heap_data_get(p1);
	...
	char* p2_data = (char*)heap_data_get(p2);
	...
	char* p3_data = (char*)heap_data_get(p3);
	...
	char* p4_data = (char*)heap_data_get(p4);
	printf("p4_data: $%04p\n", p4_data);
	strcpy(p4_data, "D:8");
	heap_dump(s1);

	heap_handle p5 = heap_alloc(s1,8);
	strcpy((char*)heap_data_get(p5), "E:8");
	heap_handle p6 = heap_alloc(s1,8);
	char* p6_data = (char*)heap_data_get(p6);
	strcpy(p6_data, "F:8");
	heap_handle p7 = heap_alloc(s1,16);
	char* p7_data = (char*)heap_data_get(p7);
	strcpy(p7_data, "G:16");
	heap_handle p8 = heap_alloc(s1,8);
	char* p8_data = (char*)heap_data_get(p8);
	strcpy(p8_data, "H:8");
	heap_dump(s1);
