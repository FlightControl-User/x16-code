...
#Verbosed compile
java -jar "$kickc_jar" -I "$user_include" -I "$kickc_stdinclude" -L "$user_lib" -L "$kickc_stdlib"   -F "$kickc_fragment_home" -P "$kickc_platform_home" -v -t=cx16 -a -Si -Onouplift -vasmout -Xassembler=-symbolfile "$filedir/$filename" | Out-File "$user_dev/compile_$filename.log"
