...
vera_heap_data_packed_t vera_heap_get_data_packed(vera_heap_handle_t index)
{
    return vera_heap_index.data[index];
}


vram_bank_t vera_heap_data_get_bank(vera_heap_handle_t index)
{
    return (vram_bank_t)BYTE1(vera_heap_index.data[index])>>5;
}


vram_offset_t vera_heap_data_get_offset(vera_heap_handle_t index)
{
    return (vram_offset_t)vera_heap_get_data_packed(index) << 3;
}




void vera_heap_set_data(vera_heap_handle_t index, vram_bank_t vram_bank, vram_offset_t vram_offset)
{
    vera_heap_index.data[index] = vera_heap_data_pack(vram_bank, vram_offset);
}


void vera_heap_set_data_packed(vera_heap_handle_t index, vera_heap_data_packed_t data_packed)
{
    vera_heap_index.data[index] = data_packed;
}

void vera_heap_set_free(vera_heap_handle_t index)
{
    // vera_heap_handle_t size = vera_heap_index.size[index];
    vera_heap_index.size[index] |= 0x8000;
}


void vera_heap_clear_free(vera_heap_handle_t index)
{
    // vera_heap_handle_t size = vera_heap_index.size[index];
    vera_heap_index.size[index] &= 0x7FFF;
}


unsigned int vera_heap_get_free(vera_heap_handle_t index)
{
    // vera_heap_handle_t size = vera_heap_index.size[index];
    return (unsigned int)vera_heap_index.size[index] & 0x8000;
}
...
vera_heap_size_packed_t vera_heap_get_size_packed(vera_heap_handle_t index)
{
    return vera_heap_index.size[index] & 0x7FFF; // ignore free flag
}
...
vera_heap_size_t vera_heap_get_size(vera_heap_handle_t index)
{
    return vera_heap_get_size_packed(index) << 3; // free flag is automatically removed
}

void vera_heap_set_size(vera_heap_handle_t index, vera_heap_size_t size)
{
    vera_heap_index.size[index] = (vera_heap_size_packed_t)(size >> 3);
}


void vera_heap_set_size_packed(vera_heap_handle_t index, vera_heap_size_packed_t size_packed)
{
    size_packed &= 0x7FFF; // ignore free flag
    vera_heap_index.size[index] = (vera_heap_index.size[index] & 0x8000) | size_packed;
}


vera_heap_handle_t vera_heap_get_next(vera_heap_handle_t index)
{
    return vera_heap_index.next[index];
}


void vera_heap_set_next(vera_heap_handle_t index, vera_heap_handle_t next)
{
    vera_heap_index.next[index] = next;
}


vera_heap_handle_t vera_heap_get_prev(vera_heap_handle_t index)
{
    return vera_heap_index.prev[index];
}


void vera_heap_set_prev(vera_heap_handle_t index, vera_heap_handle_t prev)
{
    vera_heap_index.prev[index] = prev;
}


vera_heap_handle_t vera_heap_get_left(vera_heap_handle_t index)
{
    return vera_heap_index.left[index];
}


void vera_heap_set_left(vera_heap_handle_t index, vera_heap_handle_t left)
{
    vera_heap_index.left[index] = left;
}


vera_heap_handle_t vera_heap_get_right(vera_heap_handle_t index)
{
    return vera_heap_index.right[index];
}


void vera_heap_set_right(vera_heap_handle_t index, vera_heap_handle_t right)
{
...
vera_heap_handle_t vera_heap_list_insert_at(vera_heap_handle_t list, vera_heap_handle_t index, vera_heap_handle_t at) {
	...
	vera_heap_handle_t last = vera_heap_get_prev(at);
...
vera_heap_handle_t vera_heap_list_remove(vera_heap_handle_t list, vera_heap_handle_t index) {
	...
	if(list == vera_heap_get_next(list)) {
    ...
    vera_heap_handle_t next = vera_heap_get_next(index);
    vera_heap_handle_t prev = vera_heap_get_prev(index);
...
vera_heap_handle_t vera_heap_heap_insert_at(vera_heap_segment_index_t s, vera_heap_handle_t heap_index, vera_heap_handle_t at, vera_heap_size_packed_t size) {
	vera_heap_segment.heap_list[s] = vera_heap_list_insert_at(vera_heap_segment.heap_list[s], heap_index, at);
    vera_heap_set_size_packed(heap_index, size);
...
void vera_heap_heap_remove(vera_heap_segment_index_t s, vera_heap_handle_t heap_index) {
	vera_heap_segment.heapCount[s]--;
	vera_heap_segment.heap_list[s] = vera_heap_list_remove(vera_heap_segment.heap_list[s], heap_index);
}

vera_heap_handle_t vera_heap_free_insert(vera_heap_segment_index_t s, vera_heap_handle_t free_index, vera_heap_handle_t data, vera_heap_size_packed_t size) {
	vera_heap_segment.free_list[s] = vera_heap_list_insert_at(vera_heap_segment.free_list[s], free_index, vera_heap_segment.free_list[s]);
    vera_heap_set_data_packed(free_index, data);
    vera_heap_set_size_packed(free_index, size);
    vera_heap_set_free(free_index);
...
void vera_heap_free_remove(vera_heap_segment_index_t s, vera_heap_handle_t free_index) {
    ...
    vera_heap_clear_free(free_index);
}

vera_heap_handle_t vera_heap_idle_insert(vera_heap_segment_index_t s, vera_heap_handle_t idle_index) {
	vera_heap_segment.idle_list[s] = vera_heap_list_insert_at(vera_heap_segment.idle_list[s], idle_index, vera_heap_segment.idle_list[s]);
    vera_heap_set_data_packed(idle_index, 0);
    vera_heap_set_size_packed(idle_index, 0);
...
void heap_idle_remove(vera_heap_segment_index_t s, vera_heap_handle_t idle_index) {
	vera_heap_segment.idleCount[s]--;
	vera_heap_segment.idle_list[s] = vera_heap_list_remove(vera_heap_segment.idle_list[s], idle_index);
}
...
vera_heap_handle_t vera_heap_split_free_and_allocate(vera_heap_segment_index_t s, vera_heap_handle_t free_index, vera_heap_size_packed_t required_size) {
    ...
    vera_heap_set_data_packed(free_index, free_data + required_size);
...
#ifdef VERAHEAP_DEBUG
    printf("\n > Split free index %04x size %05x and allocate heap index %04x size %05x", free_index, vera_heap_get_size(free_index), heap_index, vera_heap_get_size(heap_index));
#endif
 ...
 */
vera_heap_size_packed_t vera_heap_can_split_free(vera_heap_handle_t free_index, vera_heap_size_packed_t required_size) {

    vera_heap_size_packed_t split_size = vera_heap_get_size_packed(free_index) - required_size;
...
vera_heap_handle_t vera_heap_allocate(vera_heap_segment_index_t s, vera_heap_handle_t free_index, vera_heap_size_packed_t required_size) 
{
	...
	if (vera_heap_can_split_free(free_index, required_size)) {
...
vera_heap_handle_t vera_heap_alloc_using_free(vera_heap_segment_index_t s, vera_heap_size_packed_t requested_size) {
	...
	do {
        ...
        free_index = vera_heap_get_next(free_index);
...
vera_heap_handle_t vera_heap_can_coalesce_left(vera_heap_segment_index_t s, vera_heap_handle_t heap_index) {
    ...
    vera_heap_data_packed_t left_offset = vera_heap_get_data_packed(left_index);
    unsigned int left_free = vera_heap_get_free(left_index);
...
vera_heap_handle_t heap_can_coalesce_right(vera_heap_segment_index_t s, vera_heap_handle_t heap_index) {
    ...
    vera_heap_data_packed_t right_offset = vera_heap_get_data_packed(right_index);
    unsigned int right_free = vera_heap_get_free(right_index);
...
vera_heap_handle_t heap_coalesce_left(vera_heap_segment_index_t s, vera_heap_handle_t left_index, vera_heap_handle_t middle_index) {
    ...
    vera_heap_set_left(middle_index, free_left);
    ...
    vera_heap_set_left(left_index, VERAHEAP_NULL);
    vera_heap_set_right(left_index, VERAHEAP_NULL);

    vera_heap_set_size_packed(middle_index, left_size + middle_size);
    vera_heap_set_data_packed(middle_index, left_offset);
...
vera_heap_handle_t heap_coalesce_right(vera_heap_segment_index_t s, vera_heap_handle_t middle_index, vera_heap_handle_t right_index) {
    ...
    vera_heap_data_packed_t middle_offset = vera_heap_get_data_packed(middle_index);
    ...
    vera_heap_set_left(right_index, free_left);
    vera_heap_set_right(right_index, free_right);

    vera_heap_set_left(free_right, right_index);
    vera_heap_set_right(free_left, right_index);

    vera_heap_set_left(middle_index, VERAHEAP_NULL);
    vera_heap_set_right(middle_index, VERAHEAP_NULL);

    vera_heap_set_size_packed(right_index, middle_size + right_size);
    vera_heap_set_data_packed(right_index, middle_offset);
 ...
 * @return heap_handle 
 */
vera_heap_handle_t vera_heap_free(vera_heap_segment_index_t s, vera_heap_handle_t free_index) 
{
	...
	vera_heap_handle_t free_offset = vera_heap_get_data_packed(free_index);
...
void vera_heap_dump_index_print(char prefix, vera_heap_handle_t list)
{
	...
	do {
		printf("%03u  %c  %c  ", index, prefix, (char)BYTE1(vera_heap_get_free(index)));
		printf("%x%04x  %05x  ", vera_heap_data_get_bank(index), vera_heap_data_get_offset(index), vera_heap_get_size(index));
