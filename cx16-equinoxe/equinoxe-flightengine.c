...
#include "equinoxe-palettes.c"
    ...
    cx16_bram_packed bram_sprites = heap_segment_bram(
        ...
        cx16_bram_pack(HEAP_SEGMENT_BRAM_SPRITES_BANK, (heap_ptr)0xA000),
        cx16_size_pack(0x2000 * HEAP_SEGMENT_BRAM_SPRITES_BANKS)
    );

    // Initialize the bram heap for dynamic allocation of the entities.
    cx16_bram_packed bram_entities = heap_segment_bram(
        HEAP_SEGMENT_BRAM_ENTITIES,
        cx16_bram_pack(HEAP_SEGMENT_BRAM_ENTITIES_BANK, (heap_ptr)0xA000),
        cx16_size_pack(0x2000 * HEAP_SEGMENT_BRAM_ENTITIES_BANKS)
    );
...
__interrupt(rom_sys_cx16) void irq_vsync() {
    ...
    if (vera_sprite_is_collision()) {
        ...
        gotoxy(0, 2);
        printf("mouse: %i %i %u       ", game.curr_mousex, game.curr_mousey, (unsigned int)cx16_mouse_status);
