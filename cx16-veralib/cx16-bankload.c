...
void main() {
    ...
    memcpy_bank_to_vram(VERA_PALETTE+32, BANK_PALETTE, 32);
    memcpy_bank_to_vram(VRAM_SPRITE, BANK_SPRITE, 64*32); // 4bpp, so only 64*64/2 (2 pixels per byte).

    vera_sprite_address(0, VRAM_SPRITE);
    vera_sprite_4bpp(0);
    vera_sprite_set_xy(0,100,100);
    vera_sprite_HFlip_off(0);
    vera_sprite_VFlip_on(0);
    vera_sprite_width_64(0);
    vera_sprite_height_64(0);
    vera_sprite_zdepth_in_front(0);
    vera_sprite_palette_offset(0,1); // The palette is loaded at +1 (0+32), as each palette block is 32 bytes;
